...
固件与资源升级均显示现有人像集合中的 `thinking.png`。
这张图片同时内置于应用分区，因此启动阶段尚未 Apply assets、
或 assets 正在擦写时，升级背景也不依赖该分区。
进度文字继续显示在圆屏下方。安装新版本时使用旧固件的升级界面，
新实现从 2.4.3 启动后的资源升级及后续固件升级起生效。

重新配网：重启进入启动/连接阶段后短按 BOOT，再连接设备热点并打开
`http://192.168.4.1`。正常运行短按切换对话，未绑定长按配网。
